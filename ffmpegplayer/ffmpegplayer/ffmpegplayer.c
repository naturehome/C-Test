...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
		...
		if (vp->frame)
		{
			static AVRational s_gvr = { 1, 1000 };
			int64_t timeStamp = av_frame_get_best_effort_timestamp(vp->frame);
			curtimeStamp = av_rescale_q(timeStamp, is->video_st->time_base, s_gvr);
		}
    ...
    if (show_status) {
        ...
        if (!last_time || (cur_time - last_time) >= 30000) {
            ...
            av_log(NULL, AV_LOG_INFO,
                "%7.2f %s:%7.3f fd=%4d aq=%5dKB vq=%5dKB sq=%5dB f=%I64d /%I64d aa=%d  \r",
                get_master_clock(is),
